...
import { addReps, getOrCreateTodayChallenge, rerollChallengeItem } from "./dailyChallenge.service.js";
...
export default async function dailyChallengeRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/daily-challenge/:id/reroll", async (request, reply) => {
    const { id } = idParamSchema.parse(request.params);
    try {
      const item = await rerollChallengeItem(fastify.prisma, request.user.sub, id);
      return reply.send(toDailyChallengeItemDto(item));
    } catch (error) {
      if (error instanceof HttpError) {
        return reply.code(error.statusCode).send({ message: error.message });
