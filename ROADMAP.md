...
- [x] **Neue Seite `/history`: vergangene Trainings nach Tag durchsuchbar, editierbar**
  ...
  - **Fenstern statt echter Pagination:** Die aus `roadmap2.md` als Scaling-Sorge identifizierte unbegrenzte Fetch-Historie ist eine bewusste, vorbestehende Architekturentscheidung des Offline-First-Ansatzes (die lokale Dexie-Kopie muss vollständig sein, damit Offline-Bearbeitung überhaupt funktioniert) und wurde nicht angetastet. Stattdessen rendert `/history` anfangs nur die letzten 14 Tages-Gruppen, mit "Weitere Tage laden"-Button für den Rest — hält den DOM bei monatelanger Nutzung klein, ohne die Offline-Cache-Semantik zu verändern.
