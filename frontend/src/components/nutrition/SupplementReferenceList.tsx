...
const RATING_STYLES: Record<SupplementRating, string> = {
  ...
  overrated: "bg-ink-800 text-ink-400",
};

export function SupplementReferenceList() {
  return (
    <div>
      <h2 className="mb-2 text-sm font-medium text-ink-400">Supplement-Referenz</h2>
      <div className="flex flex-col gap-2">
        {SUPPLEMENT_REFERENCE.map((entry) => (
          <div key={entry.name} className="rounded-lg border border-ink-800 bg-ink-900 p-3">
            <div className="mb-1 flex items-start justify-between gap-2">
              <p className="text-sm font-medium text-ink-200">{entry.name}</p>
              <span
            ...
            <p className="text-sm text-ink-400">{entry.description}</p>
            {entry.dosage !== "—" && (
              <p className="mt-1 text-xs text-ink-600">Übliche Dosierung: {entry.dosage}</p>
            )}
          </div>
        ))}
      </div>
